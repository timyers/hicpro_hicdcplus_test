...
allValidPairs_paths <- c(
      "/Users/myersta/Box/Rprojects/hichip/hicpro_hicdcplus_test/data/hicpro_output/hic_results/data/dixon_2M",
      "/Users/myersta/Box/Rprojects/hichip/hicpro_hicdcplus_test/data/hicpro_output/hic_results/data/dixon_2M_2"
     )
...
for(allValidPairs_path in allValidPairs_paths){
  output_path <- paste0(out_dir,'/',
                        gsub("^(.*[\\/])", 
                             "",
                             gsub('.hic', '.txt.gz', allValidPairs_path)
                            )
                       )
  # generate gi_list instance
  gi_list <- generate_bintolen_gi_list(bintolen_path = paste0(outdir,"/hg19_5kb_hindiii_bintolen.txt.gz"),
                                       gen = "Hsapiens", gen_ver="hg19"
                                      )
  gi_list <- add_hic_counts(gi_list, hic_path = allValidPairs_path)
